...
def non_max_suppression(
    ...
    Supports YOLO (xywh+obj+cls), YOLOv11 (anchor-free xyxy+cls), and single-class detection.
    """
    ...
    if anchor_free:
        ...
        if nc <= 0:
            zeros = torch.zeros((prediction.shape[0], prediction.shape[1], 1), device=prediction.device)
    ...
    for xi, x in enumerate(prediction):
        ...
        if labels and len(labels[xi]):
            l = labels[xi]
            v = torch.zeros((len(l), nc + (4 if anchor_free else 5)), device=x.device)
            v[:, :4] = l[:, 1:5]
            if not anchor_free:
                v[:, 4] = 1.0
                v[range(len(l)), l[:, 0].long() + 5] = 1.0
            else:
                v[range(len(l)), l[:, 0].long() + 4] = 1.0
            x = torch.cat((x, v), 0)
